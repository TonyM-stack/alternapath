...
import { getAilmentsAdmin } from "@/lib/data";
import AdminAilmentsClient from "@/components/admin/AdminAilmentsClient";
...
export default async function AdminAilmentsPage() {
  await requireAdmin();
  const ailments = await getAilmentsAdmin();
  ...
  return (
    ...
    <section className="space-y-2">
      {/* <h2 className="text-xl md:text-3xl mt-8 font-semibold text-emerald-800">
        Ailments
      </h2>
      <p className="text-sm text-slate-600">
        Add and edit ailments, slugs, and active state. Slugs should match
        the URLs used for ailment detail pages.
      </p> */}

      <AdminAilmentsClient initialAilments={ailments} />
    </section>
    </main>
    </>
  );
